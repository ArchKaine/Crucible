            ...
            try {
                ...
                if (pathname === '/api/backup') {
                    try {
                        ...
                        // Creates a hidden archive folder in the project root
                        ...
                        // Generate a safe, chronological timestamp
                        const timestamp = new Date().toISOString().replace(/T/, '_').replace(/[:.]/g, '-');
                        ...
                        return sendJSON({
                            error: `Archive failure: ${err.message}`
                        }, 200);
                    }
                }
                ...
                if (pathname === '/api/history/list') {
                    try {
                        const targetFile = path.basename(data.path);
                        const backupDir = path.join(process.cwd(), '.archive');
                        ...
                        // Read the archive and filter for files matching the current target
                        const files = fs.readdirSync(backupDir);
                        const backups = files
                        .filter(f => f.startsWith(targetFile + '_'))
                        .map(f => {
                            // Extract the timestamp from the filename format: name.ext_YYYY-MM-DD...bak
                            const timeString = f.replace(targetFile + '_', '').replace('.bak', '');
                            return {
                                filename: f,
                ...
                if (pathname === '/api/history/read') {
                    try {
                        // Safely read the raw string of the requested backup
