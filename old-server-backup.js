...
const server = http.createServer(async (req, res) => {
    ...
    // This catches /ui/scripts.js and local /styles.css
    if (pathname.startsWith('/ui/') || pathname.endsWith('.js') || pathname.endsWith('.css')) {
        // Map the request to the physical /ui directory
        ...
        try {
            if (fs.existsSync(absolutePath) && fs.lstatSync(absolutePath).isFile()) {
                ...
                console.error(`[SYSTEM] Asset missing at: ${absolutePath}`);
            }
        } catch (err) {
            console.error("[SYSTEM] Asset Load Error:", err.message);
        }
    }
    ...
    // Inside your main http.createServer callback, before your 404 fallback:
            ...
            try {
                ...
                if (pathname === '/api/git/action') {
                    ...
                    try {
                        switch (data.action) {
                            case 'init':
                                command = `git -C "${targetDir}" init`;
                                break;
                            case 'stage':
                                command = `git -C "${targetDir}" add "${data.file}"`;
                                break;
                            case 'unstage':
                                command = `git -C "${targetDir}" reset HEAD "${data.file}"`;
                                break;
                            case 'add-all':
                                command = `git -C "${targetDir}" add .`;
                                break;
                            ...
                            case 'push':
                                command = `git -C "${targetDir}" push -u origin HEAD`;
                                break;
                            case 'pull':
                ...
                if (pathname === '/api/git/auth') {
                    const targetDir = data.dir || process.cwd();
                    try {
                        // Retrieve the current remote URL (Blindfolded)
                        const {
                            stdout: remoteUrl
                        } = await execPromise(`git -C "${targetDir}" remote get-url origin`, {
                            env: {
                        ...
                        if (cleanUrl.startsWith('https://')) {
                            // Strip existing token/auth if present
                            cleanUrl = cleanUrl.replace(/https:\/\/[^@]+@/, 'https://');

                            // Extract the GitHub username from the URL path
                            // e.g., https://github.com/ArchKaine/Crucible.git -> ArchKaine
                            const urlParts = cleanUrl.split('/');
                            const githubUser = urlParts[3];

                            // Inject the full explicit credentials: username:token@
                            const authUrl = cleanUrl.replace('https://', `https://${githubUser}:${data.token}@`);

                            // Set the new URL (Blindfolded)
